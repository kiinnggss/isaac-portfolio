...
3 packets transmitted, 3 packets received, 0.0% packet loss, round-trip min/avg/max = ${duration - 1}/${duration}/${duration + 1} ms`;
          ...
          // Static host fallback calculation
...
3 packets transmitted, 3 received, 0% packet loss, rtt avg: ${duration}ms`;
  ...
  return (
    <div className="rounded-xl bg-white border border-slate-200 p-6 sm:p-7 shadow-xs">
        ...
        <div className="flex flex-wrap gap-1.5">
          {presetCommands.map((preset) => (
            <button
              ...
              className="px-2.5 py-1 rounded-md text-xs font-mono-tech bg-slate-100 hover:bg-slate-200 text-slate-700 transition-colors"
            >
              {preset.label}
            </button>
          ))}
      ...
      {/* Terminal Window */}
      <div className="mt-5 rounded-xl bg-slate-950 border border-slate-800 shadow-md overflow-hidden flex flex-col h-96">
        ...
        <div className="px-4 py-2.5 bg-slate-900 border-b border-slate-800 flex items-center justify-between text-xs text-slate-400">
          <div className="flex items-center gap-2">
            ...
            <span className="ml-2 font-mono-tech text-[11px] text-slate-300">
              isaac@lagos-edge:~
            </span>
          </div>
        ...
        <div className="flex-1 p-4 overflow-y-auto font-mono-tech text-xs space-y-2 leading-relaxed text-slate-200">
          {lines.map((line) => (
            <div key={line.id}>
              {line.type === "input" && (
        ...
        >
          <span className="font-mono-tech text-emerald-400 text-xs pl-1">
            isaac@lagos-edge:~$
          </span>
          <input
            type="text"
